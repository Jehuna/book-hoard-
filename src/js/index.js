$(document).ready(function () {
  ...
  $(document).on("click", "#addComment", function () {
    let form = {
      body: $("#commentTitle").val(),
      time: timeDate(),
    };
    createPost(form, renderComments, () => $("#commentTitle").val(""));
  });
  ...
  function getBooksData(collection) {
    ...
    database.ref(collection).on("value", (res) => {
      ...
      let chooseCatalogBookData = allBookData.filter(
        (book) => book.catalog == localStorage.getItem("catalog")
      );
      let bestellerBookData = allBookData.filter(
        (book) => book.catalog === "2"
      );
      ...
      renderSlider(
        ...
        ".product-prev",
        ".product-next",
        chooseCatalogBookData.length > 0 ? chooseCatalogBookData : allBookData
      );
      renderSlider(
        ".besteller-slider",
        ".besteller-prev",
        ...
        return `
                    <div>
                        <div class="jumbotron d-flex bg-transparent justify-content-between shadow mt-5"
                            ...
                            <span class="${
                              !book.is_new && "d-none"
                            } badge bg-danger text-light">New</span>
        ...
        return `
                <div>
                    <div class="card shadow position-relative" style="width: 184px;">
                        <span class="${
                          !book.is_new && "d-none"
                        } badge badge-danger p-1 position-absolute">New</span>
                        <img width="240" height="190"
                            src="${book.img_url}"
                            style="object-fit: cover;"
                            class="card-img-top" alt="${book.title}">
                        <div class="card-body p-3 text-center">
                            <h5 class="card-title">${book.title.slice(
                              0,
                              10
                            )}...</h5>
                            <h6 class="card-title">${book.author.slice(
                              0,
                              10
                            )}..</h6>
                            <button data-id="${
                              book.id
                            }" class="btn bg-orange text-white btn-block readMoreBook ">Read more</a>
                        </div>
                    </div>
                </div>
    `;
  ...
  function renderComments(arr) {
    $(".comment-list").html(
      arr
        ...
        .map(
          (post) => `
              <div class="comment-card jumbotron p-4 bg-white shadow-sm my-5">
              <div class="d-flex h6 font-weight-bold">
                <p>anonim</p>
                <p class="mx-3">${post?.time ?? timeDate()} today</p>
              </div>
              <div class="display-5">
                ${post.body}
              </div>
